...
namespace RecycleWeb.Member
{
    public partial class Withdrawal : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                string temp = WebApiUtil.Unlink(KAKAO_KAPI_URI, GET_UNLINK_RESOURCE, Session["accessToken"].ToString());

                Dictionary<string, string>  param = new Dictionary<string, string>();
                param.Add("producerIdx ", Session["producerIdx"].ToString());
                param.Add("leaveReasonIdx", "0");
                param.Add("leaveComment", string.Empty);

                WebApiUtil.RestRequest(APP_SERVER_URI, USER_LEAVE, param);
